<template>
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M10 18C14.4183 18 18 14.4183 18 10C18 5.58172 14.4183 2 10 2C5.58172 2 2 5.58172 2 10C2 14.4183 5.58172 18 10 18ZM10 20C15.5228 20 20 15.5228 20 10C20 4.47715 15.5228 0 10 0C4.47715 0 0 4.47715 0 10C0 15.5228 4.47715 20 10 20Z"
          fill="#6F767E"
        />
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M10 4C10.5523 4 11 4.44772 11 5V9.58579L13.2071 11.7929C13.5976 12.1834 13.5976 12.8166 13.2071 13.2071C12.8166 13.5976 12.1834 13.5976 11.7929 13.2071L9.58579 11C9.21071 10.6249 9 10.1162 9 9.58579V5C9 4.44772 9.44772 4 10 4Z"
          fill="#6F767E"
        />
    </svg>
</template>